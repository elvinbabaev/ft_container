...
void construtor() {
	...
	std::cout << RED << "s_v_fill.size() = " << RESET << s_v_fill.size() << RED << "   s_v_fill.capacity() = " << RESET
	          << s_v_fill.capacity() << std::endl;
	std::cout << GREEN << "f_v_fill.size() = " << RESET << f_v_fill.size() << GREEN << "   f_v_fill.capacity() = "
	          << RESET << f_v_fill.capacity() << std::endl;
	...
	check_assign(s_v_fill, f_v_fill);
	...
	ft::vector<int> f_v_range(f_v_fill.begin(), f_v_fill.end());
	...
	check_assign(s_v_range, f_v_range);
	...
	check_assign(s_v_range, f_v_range);
	check_assign(s_v_fill, f_v_fill);
	check_assign(s_v_copy, f_v_copy);
	check_assign(s_v_def, f_v_def);
}
...
void assign_operator() {
	...
	check_assign(s_v_tmp, f_v_tmp);
	check_assign(s_v_assign, f_v_assign);
}
...
void capacity() {
	...
	std::cout << MAGENTA << "s_v.capacity() = " << RESET << s_v.capacity() << std::endl;
	std::cout << CYAN << "f_v.capacity() = " << RESET << f_v.capacity() << std::endl;
	std::vector<std::string>::iterator s_it = s_v.begin();
	ft::vector<std::string>::iterator f_it = f_v.begin();
	std::cout << MAGENTA << "vector s_v: value = " << RESET;
	for (; s_it != s_v.end(); ++s_it) {
		std::cout << *s_it << " ";
	}
...
void swap() {
	...
	std::vector<int> s_v(3, 100);   // three ints with a value of 100
	std::vector<int> s_v2(3, 400);   // three ints with a value of 100
	ft::vector<int> f_v(3, 100);
	ft::vector<int> f_v2(3, 400);
	std::cout << CYAN << "f_v[i] = " << RESET;
	for (std::vector<int>::size_type i = 0; i != f_v.size(); ++i) {
		std::cout << f_v[i] << " ";
	}
...
void relational_operators() {
	{
		...
		if (foo == bar) std::cout << CYAN << "foo and bar are equal\n";
		if (foo != bar) std::cout << CYAN << "foo and bar are not equal\n";
